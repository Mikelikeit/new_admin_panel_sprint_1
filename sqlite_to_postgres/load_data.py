...
def extract_data(sqlite_cursor: sqlite3.Cursor,
    ...
    for replace in DifferentColumn:
        query_slt_sqlite = query_slt_sqlite.replace(replace.name, replace.value)
    sqlite_cursor.execute(query_slt_sqlite)
    while results := sqlite_cursor.fetchmany(BATCH_SIZE):
        yield results


def _rename_column(row: dict) -> dict:
    for name in DifferentColumn:
        if row.get(name.value):
            row[name.name] = row.get(name.value)
            del row[name.value]
    return row
...
def transform_data(sqlite_cursor: sqlite3.Cursor,
    ...
    for batch in extract_data(sqlite_cursor, table_name, column_names):
        yield [table_fabric[table_name](**_rename_column(dict(row))) for row in batch]
